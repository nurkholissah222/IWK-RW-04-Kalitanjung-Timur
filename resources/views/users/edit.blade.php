<x-app-layout>
    <div class="p-6 lg:p-8">
        <div class="max-w-2xl mx-auto">
            ...
            <div class="mb-8">
                <h1 class="text-4xl font-black text-slate-900 tracking-tight">Edit Akun User</h1>
                <p class="text-slate-600 mt-2 font-medium">Perbarui informasi akun Petugas RT atau Admin RW.</p>
            </div>
            ...
            <div class="bg-white border border-slate-200 rounded-[2.5rem] shadow-2xl shadow-slate-200/50 p-10">
                <form action="{{ route('users.update', $user->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <!-- Email -->
                    <div>
                        <label class="block text-sm font-black uppercase tracking-widest text-slate-500 mb-2 ml-1">Email Aktif</label>
                        <input type="email" name="email" value="{{ old('email', $user->email) }}" required 
                               class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 placeholder-slate-400 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all duration-300 outline-none">
                        @error('email') <p class="mt-2 text-sm text-rose-500 font-bold">{{ $message }}</p> @enderror
                    </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <select name="role" id="role" required class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all duration-300 outline-none appearance-none cursor-pointer">
                                    <option value="RT" {{ old('role', $user->role) == 'RT' ? 'selected' : '' }}>Bendahara RT</option>
                                    <option value="RW" {{ old('role', $user->role) == 'RW' ? 'selected' : '' }}>Bendahara RW</option>
                                </select>
                        ...
                        <div id="rt_id_container">
                            ...
                            <div class="relative">
                                <select name="rt_id" id="rt_id" class="w-full bg-slate-50 border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all duration-300 outline-none appearance-none cursor-pointer">
                                    @foreach($rts as $rt)
                                        <option value="{{ $rt->id }}" {{ old('rt_id', $user->rt_id) == $rt->id ? 'selected' : '' }}>RT {{ $rt->nomor_rt }}</option>
                                    @endforeach
                                </select>
                                <div class="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-slate-400">
                                    <i class="fas fa-chevron-down text-xs"></i>
                    ...
                    <div class="p-8 bg-indigo-50 border border-indigo-100 rounded-3xl">
                        <h4 class="text-sm font-black uppercase tracking-widest text-indigo-600 mb-6 flex items-center gap-3">
                            <i class="fas fa-key"></i>
                            Ganti Password (Opsional)
                        </h4>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div>
                                <label class="block text-sm font-black uppercase tracking-widest text-slate-500 mb-2 text-[10px]">Password Baru</label>
                                <div class="relative group">
                                    <input type="password" name="password" 
                                           class="w-full bg-white border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 placeholder-slate-400 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all duration-300 outline-none" placeholder="Isi jika ingin diubah">
                            ...
                            <div>
                                <label class="block text-sm font-black uppercase tracking-widest text-slate-500 mb-2 text-[10px]">Konfirmasi</label>
                                <div class="relative group">
                                    <input type="password" name="password_confirmation" 
                                           class="w-full bg-white border border-slate-200 rounded-2xl px-5 py-3.5 text-slate-900 placeholder-slate-400 focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all duration-300 outline-none" placeholder="Ulangi password">
                                    <button type="button" onclick="togglePassword(this)" class="absolute right-5 top-1/2 -translate-y-1/2 text-slate-400 hover:text-indigo-600 transition-colors focus:outline-none">
                                        <i class="fas fa-eye"></i>
                                    </button>
                                </div>
                    ...
                    <div class="pt-10 flex justify-end gap-4 border-t border-slate-100">
                        ...
                        <button type="submit" class="px-10 py-3.5 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase tracking-widest text-xs rounded-2xl shadow-xl shadow-indigo-500/20 hover:shadow-indigo-500/40 transition-all duration-300 transform hover:-translate-y-1 active:scale-[0.98]">Update Akun</button>
                    </div>
                </form>
            </div>
        </div>
